...
#!/bin/sh
# Generated by epic-script. Supplies the RandomX library path, the plugin directory and a
...
#!/bin/sh
# Generated by epic-script. Adds the Epic binaries to PATH, once.
case ":\${PATH}:" in
	*:"$_dir_expr":*) ;;
...
ENVSH
	...
	if [ -d "$HOME/.config/fish" ]; then
		...
		cat >"$HOME/.config/fish/conf.d/epic-script.fish" <<FISHSH
# Generated by epic-script.
if not contains "$BIN_DIR" \$PATH
	set -x PATH "$BIN_DIR" \$PATH
end
FISHSH
		record "$HOME/.config/fish/conf.d/epic-script.fish"
...
write_uninstaller() {
...

	{
		cat <<'UNHEAD'
#!/bin/sh
# Generated by epic-script. Removes what the installer added, and nothing else.
#
...
UNHEAD
		...
		grep '^rcline:' "$RECEIPT" 2>/dev/null | sed 's/^rcline://' | sort -u | while IFS= read -r _rc; do
			cat <<UNRC
if [ -f "$_rc" ]; then
	grep -v -F '$_meta_expr/env' "$_rc" | grep -v '^# Added by epic-script\$' > "$_rc.epic-tmp" &&
...
parse_args() {
	while [ $# -gt 0 ]; do
		...
		-h | --help)
			...
			;;
		-V | --version)
			say_bare "epic-script $INSTALLER_VERSION"
			exit 0
			;;
		*)
...
main() {
	...
	WORK_TMP="$(mktemp -d "${TMPDIR:-/tmp}/epic-script.XXXXXX")" ||
